	...
	-- IDs générés le : 07/04/15 à 21:37:21
	...
	-- SALON/CUISINE
		-- veille automatique Multimedia	
			GEA.add({"Sensor-", id["TV"], 40}, 5*60, "Consommation #value# inférieur à 40W depuis 5 minutes",  {{"turnOff", id["SUBWOFER"]}})
			GEA.add({"Sensor+", id["TV"], 40}, 30, "",  {{"turnOn", id["SUBWOFER"]}})
			GEA.add({"Global", "Lave_Vaisselle", "En Attente"}, -1, "Le Lave_Vaisselle est arrêté depuis #duration#",{{"Repeat"},sendsms_vince})
			GEA.add({"Sensor-", id["LAVE_VAISSELLE"], 3}, 30*60, "Le Lave_Vaisselle est arrêté depuis #duration#", {{"Global", "Lave_Vaisselle", "Arret"},sendsms_vince})   	
	...
	-- SDB
		-- Eteindre automatiquement le local technique après 10 mn
			GEA.add(id["PLAFONIER_SDB"], 50*60, "Extinction automatique des lumieres de la SDB", {{"turnOff"}}) 
		-- Allumage automatique à l'ouverture de la porte
		...
		-- === Lave-Linge == --
			...
			GEA.add({"Sensor-", id["LAVE_LINGE"], 2.8}, 30*60, "Le lave-linge est arrêté depuis #duration#", {{"Global", "Lave_Linge", "Arret"}})   	
		...
		-- Gestion chauffage lors ouverture/fermeture de la fenetre	
			...
			GEA.add(id["GRANDE_FENETRE"], -1, "Fenetre chambre fermée", {{"Inverse"},{"VirtualDevice", id["PANNEAU_CHAUFFAGE_SDB"], "5"}})
	...
	--WC
		-- Eteindre automatiquement après 10 mn
	...
	--ENTREE
		...
		-- Sureillance de la porte
			GEA.add( id["PORTE_ENTRE_COULOIR"], 2*60, "", {
				{"Global", "sms", hc2prefixe .."Porte entrée ouverte depuis #duration#"},sendsms_vince, sendsms_solene,
				{"Global", "mysms", hc2prefixe .."Porte entrée ouverte depuis #duration#"},sendfreesms,{"Repeat"}
			}) 
	...
	--CHAMBRE
		...
		-- Gestion chauffage lors ouverture/fermeture de la fenetre	
			...
			GEA.add(id["FENETRE"], -1, "Fenetre chambre fermée", {{"Inverse"},{"VirtualDevice", id["PANNEAU_CHAUFFAGE_CHAMBRE"], "5"}})
	...
	--SYSTEM
		-- Gestion des présences
			...
			GEA.add({"Global", "presence_vince", "1"}, 30, "Présence vincent")
			GEA.add({"Global", "presence_so", "1"}, 30, "Présence solene")
	...
	--REVEIL DES NOEUX MORTS MODULES SUR SECTEUR	
		GEA.add({"Dead", id["PLAQUE_ELECTRIQUE"]}, 1*60, "Le périphérique #name# ne répond plus", {{"WakeUp", id["PLAQUE_ELECTRIQUE"]}})
		GEA.add({"Dead", id["FRIGO"]}, 1*60, "Le périphérique #name# ne répond plus", {{"WakeUp", id["FRIGO"]}})
